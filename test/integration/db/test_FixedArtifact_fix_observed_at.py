...
def tearDown():
    with session_scope() as session:
        records = session.query(Vulnerability).filter(Vulnerability.id=='FA_TEST-1234').all()
...
def test_FixedArtifact_fix_observed_at_behavior(anchore_db):
    try:
        ...
        with session_scope() as session:
            records = session.query(FixedArtifact).filter(FixedArtifact.vulnerability_id=='FA_TEST-1234').all()
            for record in records:
                p = record.name
                logger.info("FOUND: record {} - version {} - fix_observed_at - {}".format(record.vulnerability_id, record.version, record.fix_observed_at))
                if record.version == 'None' and record.fix_observed_at is not None:
                    logger.info("FAIL: record shows no fix version, but fix_observed_at set")
                    assert(False)
                elif record.version is not None and record.version != 'None' and record.fix_observed_at is None:
                    logger.info("FAIL: record shows fix version, but fix_observed_at is null")
                    assert(False)
                else:
                    logger.info("SUCCESS: record {} is correct at this phase ({} - {})".format(p, record.version, record.fix_observed_at))
        ...
        with session_scope() as session:
            ...
            record = session.query(FixedArtifact).filter(FixedArtifact.vulnerability_id=='FA_TEST-1234', FixedArtifact.name=='fa_testpkg2').one()
            timestamps['fa_testpkg2'] = record.fix_observed_at

        with session_scope() as session:
            for p in ['fa_testpkg1', 'fa_testpkg2']:
                record = session.query(FixedArtifact).filter(FixedArtifact.vulnerability_id=='FA_TEST-1234', FixedArtifact.name==p).one()
                if record.fix_observed_at != timestamps[p]:
                    logger.info("FAIL: {} timestamp in DB is not equal to original set ({} != {})".format(p, record.fix_observed_at, timestamps[p]))
                    assert(False)
                else:
                    logger.info("SUCCESS: record {} is correct at this phase ({} - {})".format(p, record.version, record.fix_observed_at))
            

        logger.info("TESTING: state after further update of elements related to fix version")
        with session_scope() as session:
            records = session.query(FixedArtifact).filter(FixedArtifact.vulnerability_id=='FA_TEST-1234').all()
            for record in records:
                record.version = "2.0"
