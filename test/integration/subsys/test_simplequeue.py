...
def test_qs(anchore_db):
    ...
    simplequeue.create_queue(singleton_queue, max_outstanding_msgs=1, visibility_timeout=10)
...
def test_visibility_timeout(test_qs):
    ...
    assert min_approx_timeout <= msg['visible_at'] <= max_approx_timeout, 'Msg visible time, {}, outside expected range {} - {}'.format(msg.get('visible_at'), min_approx_timeout, max_approx_timeout)

    logger.info('Updating timeout: {}'.format(msg['visible_at'].isoformat()))
    ts = simplequeue.update_visibility_timeout(multi_queue, receipt_handle=msg['receipt_handle'], visibility_timeout=20)
