...
from test.integration.services.policy_engine.engine.policy.gates import GateUnitTest, anchore_db, cls_test_data_env, cls_no_feeds_test_env
...
class ImageMetadataGateTest(GateUnitTest):
    ...
    def test_imagemetadata_validator(self):
        ...
        for attr, result in list(test.items()):
            ...
            try:
                t, gate, test_context = self.get_initialized_trigger(ImageMetadataAttributeCheckTrigger.__trigger_name__, attribute=attr, check='=', value=val)
                if not expect_ok and t:
                    self.assertEqual(expect_ok, bool(t))
            except:
                if not expect_ok:
                    logger.info(('Caught expected exception on invalid attr name: {}'.format(attr)))
                else:
                    raise
