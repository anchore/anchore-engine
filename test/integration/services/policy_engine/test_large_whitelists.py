...
def test_whitelists(large_whitelist, test_data_env_with_images_loaded):
    ...
    assert evaluation.json() == no_index_evaluation.json(), 'Index vs non-indexed returned different results'
    assert no_index_evaluation is not None, 'Got None eval'
    logger.info(('Non-indexed Evaluation: {}'.format(json.dumps(evaluation.json(), indent=2))))
...
def test_regexes(large_whitelist, test_data_env_with_images_loaded):
    ...
    assert evaluation.json()['bundle_decision']['policy_decisions'][0]['decisions'] == evaluation2.json()['bundle_decision']['policy_decisions'][0]['decisions']
    logger.info(('Evaluation: {}'.format(json.dumps(evaluation.json(), indent=2))))
    open_ssl_wl_match = {
        "action": "go",
        "rule": {
    ...
    assert open_ssl_wl_match in evaluation.json()['bundle_decision']['policy_decisions'][0]['decisions']
    assert len([x for x in evaluation.json()['bundle_decision']['policy_decisions'][0]['decisions'] if x['match'].get('whitelisted', {}).get('matched_rule_id', '') in ['testinserted1', 'testinserted2', 'testinserted3']]) >= 1
