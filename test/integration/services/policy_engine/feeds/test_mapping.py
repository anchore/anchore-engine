...
vuln_mapper = VulnerabilityFeedDataMapper(feed_name='vulnerabilities', group_name='debian:9', keyname='Name')
npm_mapper = NpmPackageDataMapper(feed_name='packages', group_name='npm', keyname='name')
gem_mapper = GemPackageDataMapper(feed_name='packages', group_name='gem', keyname='name')
...
def test_vuln_overflow():
    ...
    print(('Truncated description length: {} from {}'.format(len(r.description),
                                                            len(long_cve['Vulnerability']['Description']))))
    assert r.id == test_cve['Vulnerability']['Name']
    assert r.namespace_name == test_cve['Vulnerability']['NamespaceName']
