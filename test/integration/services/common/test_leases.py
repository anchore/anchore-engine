...
def test_serial_lock(anchore_db):
    ...
    assert release1 is None, 'No one else should have the lock, should have released cleanly'
    ...
    bad_release = db_locks.release_lease(r['id'], r['held_by'], r['epoch']) # This shouldn't work, old epoch and wrong id, should return the actual holder
    assert bad_release is None, 'Epoch and id should be stale and return the actual lock holder, not None: {}'.format(bad_release)
...
def test_expiration(anchore_db):
    ...
    assert db_locks.release_lease(l2['id'], l2['held_by'], l2['epoch']) is None, 'No lock should be held'
