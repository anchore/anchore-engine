import subprocess
import unittest
from anchore_engine.services.policy_engine.engine.util.apk import compare_versions
...
class TestApkVersionHandling(unittest.TestCase):
    versions = [
        ...
        ('1.0', '1.0_alpha', {'lt': True, 'gt': False, 'eq': False}),  # There is a bug in APK here, where it treats comparison of 1.0 vs 1.0_alpha differently than 1.1 vs 1.1_alpha
        ...
        ('1.1_blarf', '1.1_alpha', {'lt': True, 'gt': False, 'eq': False}),
    ...
    def setUpClass(cls):
        if enable_training:
            ...
            container_id = subprocess.check_output(['docker', 'run', '-tid', 'alpine', 'tail', '-f', '/dev/null'])
