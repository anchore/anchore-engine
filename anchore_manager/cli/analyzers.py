...
@click.option('--parent-digest', help="Specify parent digest (ex: sha256:<64 hex characters>, default: <supplied or calculated image digest>")
@click.option('--image-id', help="Specify image ID (ex: <64 hex characters>)")
@click.option('--tag', help="Specify image full tag (ex: docker.io/somerepo:latest)", required=True)
@click.option('--account-id', help="Specify anchore account ID of the image (ex: admin, default: admin)")
@click.option('--manifest', type=click.Path(exists=True), help="Specify location of image manifest file (schema v2, not manifest list)")
@click.option('--dockerfile', type=click.Path(exists=True), help="Specify location of Dockerfile used to build image")
@click.option('--created_at', help="UNIX timestamp to store when image was created (ex: 1560454880, default: now)")
@click.option('--annotation', nargs=1, multiple=True, help="Set annotation key/val (ex: foo=bar)")
def exec(docker_archive, anchore_archive, digest, parent_digest, image_id, tag, account_id, manifest, dockerfile, created_at, annotation):
    """
