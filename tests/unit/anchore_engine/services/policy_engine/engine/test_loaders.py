...
class TestLoadingJavaCPEs:
    ...
    def test_extract_java_syft_cpes_returns_cpes(self):
        cpes = ImageLoader(self.ANALYSIS_DATA).extract_syft_cpes({}, self.ANALYSIS_DATA["package_list"]["pkgs.java"]["base"], get_image(), "java")
        assert cpes is not None

    def test_fuzzy_java_cpes_returns_cpes(self):
        fuzzy_cpes = ImageLoader(self.ANALYSIS_DATA).get_fuzzy_java_cpes(self.ANALYSIS_DATA, {}, get_image())
...
class TestLoadingPythonCPEs:
    ...
    def test_extract_python_syft_cpes_returns_cpes(self):
        cpes = ImageLoader(self.ANALYSIS_DATA).extract_syft_cpes({}, self.ANALYSIS_DATA["package_list"]["pkgs.python"][
            "base"], get_image(), "python")
        assert cpes is not None

    def test_fuzzy_python_cpes_returns_cpes(self):
        fuzzy_cpes = ImageLoader(self.ANALYSIS_DATA).get_fuzzy_python_cpes(self.ANALYSIS_DATA, {}, get_image())
        assert fuzzy_cpes is not None
...
class TestLoadingGemsCPEs:
    ...
    def test_extract_gems_syft_cpes_returns_cpes(self):
        cpes = ImageLoader(self.ANALYSIS_DATA).extract_syft_cpes({}, self.ANALYSIS_DATA["package_list"]["pkgs.gems"][
            "base"], get_image(), "gem")
        assert cpes is not None

    def test_fuzzy_gem_cpes_returns_cpes(self):
        fuzzy_cpes = ImageLoader(self.ANALYSIS_DATA).get_fuzzy_gem_cpes(self.ANALYSIS_DATA, {}, get_image())
        assert fuzzy_cpes is not None
...
class TestLoadingNpmsCPEs:
    ...
    def test_extract_npms_syft_cpes_returns_cpes(self):
        cpes = ImageLoader(self.ANALYSIS_DATA).extract_syft_cpes({}, self.ANALYSIS_DATA["package_list"]["pkgs.npms"][
            "base"], get_image(), "npm")
        assert cpes is not None

    def test_fuzzy_npm_cpes_returns_cpes(self):
        fuzzy_cpes = ImageLoader(self.ANALYSIS_DATA).get_fuzzy_npm_cpes(self.ANALYSIS_DATA, {}, get_image())
        assert fuzzy_cpes is not None