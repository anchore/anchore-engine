...
class PackageCheckGateTest(GateUnitTest):
    ...
    def test_blacklist(self):
        ...
        # Match
        t, gate, test_context = self.get_initialized_trigger(BlackListTrigger.__trigger_name__, name='libc6', version='2.24-11+deb9u4')
        ...
        # No match
        t, gate, test_context = self.get_initialized_trigger(BlackListTrigger.__trigger_name__, name='libc6', version='2.24-10+deb9u4')
        db = get_thread_scoped_session()
        test_context = gate.prepare_context(self.test_image, test_context)
        t.evaluate(self.test_image, test_context)
        logger.info(('Fired: {}'.format(t.fired)))
        ...
        # Match, name only
        t, gate, test_context = self.get_initialized_trigger(BlackListTrigger.__trigger_name__, name='libc6')
        db = get_thread_scoped_session()
        test_context = gate.prepare_context(self.test_image, test_context)
        t.evaluate(self.test_image, test_context)
        logger.info(('Fired: {}'.format(t.fired)))
        ...
        # No match
        t, gate, test_context = self.get_initialized_trigger(BlackListTrigger.__trigger_name__, name='libc-not-real')
        db = get_thread_scoped_session()
        test_context = gate.prepare_context(self.test_image, test_context)
        t.evaluate(self.test_image, test_context)
        logger.info(('Fired: {}'.format(t.fired)))
    ...
    def test_pkg_required(self):
        ...
        try:
            ...
            t, gate, test_context = self.get_initialized_trigger(RequiredPackageTrigger.__trigger_name__, name='libc6', version='2.24-11+deb9u4', version_match_type='exact')
            ...
            t, gate, test_context = self.get_initialized_trigger(RequiredPackageTrigger.__trigger_name__, name='libc6', version='2.24-10+deb9u4', version_match_type='minimum')
            test_context = gate.prepare_context(image, test_context)
            t.evaluate(image, test_context)
            logger.info(('Fired: {}'.format(t.fired)))
            self.assertEqual(0, len(t.fired))
            ...
            t, gate, test_context = self.get_initialized_trigger(RequiredPackageTrigger.__trigger_name__, name='libc6', version='2.24-10+deb9u4')
            test_context = gate.prepare_context(image, test_context)
            t.evaluate(image, test_context)
            logger.info(('Fired: {}'.format(t.fired)))
            self.assertEqual(1, len(t.fired))
            ...
            t, gate, test_context = self.get_initialized_trigger(RequiredPackageTrigger.__trigger_name__, name='libc6', version='2.24-10+deb9u4', version_match_type='exact')
            test_context = gate.prepare_context(image, test_context)
            t.evaluate(image, test_context)
            logger.info(('Fired: {}'.format(t.fired)))
            self.assertEqual(1, len(t.fired))
            ...
            t, gate, test_context = self.get_initialized_trigger(RequiredPackageTrigger.__trigger_name__, name='libc6', version='2.24-15+deb9u4', version_match_type='minimum')
            test_context = gate.prepare_context(image, test_context)
            t.evaluate(image, test_context)
            logger.info(('Fired: {}'.format(t.fired)))
            self.assertEqual(1, len(t.fired))
            ...
            t, gate, test_context = self.get_initialized_trigger(RequiredPackageTrigger.__trigger_name__, name='libc-not-installed')
            test_context = gate.prepare_context(image, test_context)
            t.evaluate(image, test_context)
            logger.info(('Fired: {}'.format(t.fired)))
            self.assertEqual(1, len(t.fired))
            ...
            t, gate, test_context = self.get_initialized_trigger(RequiredPackageTrigger.__trigger_name__, name='libc-not-installed', version='1.1.0', version_match_type='exact')
            test_context = gate.prepare_context(image, test_context)
            t.evaluate(image, test_context)
            logger.info(('Fired: {}'.format(t.fired)))
            self.assertEqual(1, len(t.fired))
            ...
            t, gate, test_context = self.get_initialized_trigger(RequiredPackageTrigger.__trigger_name__, name='libc-not-installed', version='1.1.0', version_match_type='minimum')
            test_context = gate.prepare_context(image, test_context)
            t.evaluate(image, test_context)
            logger.info(('Fired: {}'.format(t.fired)))
            self.assertEqual(1, len(t.fired))
    ...
    def test_verifytrigger(self):
        ...
        test_context = gate.prepare_context(self.test_image, test_context)
        t.evaluate(self.test_image, test_context)
        self.assertGreater(len(t.fired), 0) # 1890 is the exact number for this specific image
        ...
        t, gate, test_context = self.get_initialized_trigger(VerifyTrigger.__trigger_name__, only_directories='/usr/share/doc/tzdata/', check='changed')
        db = get_thread_scoped_session()
        db.refresh(self.test_image)
        test_context = gate.prepare_context(self.test_image, test_context)
        t.evaluate(self.test_image, test_context)
        ...
        t, gate, test_context = self.get_initialized_trigger(VerifyTrigger.__trigger_name__, only_packages='tzdata')
        db = get_thread_scoped_session()
        db.refresh(self.test_image)
        test_context = gate.prepare_context(self.test_image, test_context)
        t.evaluate(self.test_image, test_context)
        ...
        logger.info('Specific pkg, with issues')
        t, gate, test_context = self.get_initialized_trigger(VerifyTrigger.__trigger_name__, only_packages='zlib1g')
        db = get_thread_scoped_session()
        db.refresh(self.test_image)
        test_context = gate.prepare_context(self.test_image, test_context)
        t.evaluate(self.test_image, test_context)
        ...
        logger.info('Specific pkg, with issues')
        t, gate, test_context = self.get_initialized_trigger(VerifyTrigger.__trigger_name__, only_packages='util-linux')
        db = get_thread_scoped_session()
        db.refresh(self.test_image)
        test_context = gate.prepare_context(self.test_image, test_context)
        t.evaluate(self.test_image, test_context)
        ...
        t, gate, test_context = self.get_initialized_trigger(VerifyTrigger.__trigger_name__, only_packages='findutil-notfound')
        db = get_thread_scoped_session()
        db.refresh(self.test_image)
        test_context = gate.prepare_context(self.test_image, test_context)
        t.evaluate(self.test_image, test_context)
