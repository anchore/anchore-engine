    ...
    try:
        ...
        try:
            ...
            if e.response.get('Code') == 'BucketNotEmpty':
                ...
                logger.warn('Error cleaning up bucket {}, may result in other test failures'.format(bucket_name))
...
def swift_container():
    ...
    try:
        ...
        logger.info('Deleting/cleanup a swift container: {}'.format(test_swift_container))
        try:
            client.delete(container=container_name)
        except Exception as e:
            logger.exception('Container cleanup exception')
            logger.warn('Error cleaning up swift container {}, may result in other test failures'.format(container_name))
