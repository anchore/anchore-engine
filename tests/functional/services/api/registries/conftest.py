...
from tests.functional.services.utils.http_utils import RequestFailedError, http_post, http_del
...
def add_and_teardown_registry(request):
    ...
    add_registry_resp = http_post(['registries'], registry_payload, config=request.param)
    if add_registry_resp.code != 200:
        raise RequestFailedError(add_registry_resp.url, add_registry_resp.code, add_registry_resp.body)
