...
from tests.functional.services.api.images import get_image_id, get_image_tag, \
...
class TestImagesByIDAPIGetReturns200:
    ...
    @pytest.mark.parametrize('query', [pytest.param({}, id='noparams'),
    ...
    def test_get_image_content_files(self, add_alpine_latest_image):
        ...
        resp = http_get(['images', 'by_id', image_id, 'content', 'files'], config=api_conf)
    ...
    def test_get_image_content_java(self, add_alpine_latest_image):
        ...
        resp = http_get(['images', 'by_id', image_id, 'content', 'java'], config=api_conf)
