...
from tests.functional.utils.http_utils import http_get, RequestFailedError
...
def wait_for_image_to_analyze(image_id, api_conf: callable):
    ...
    while status != 'analyzed' and time.time() - start_time_sec < WAIT_TIMEOUT_SEC:
        ...
        if status != 'analyzed':
            _logger.info('Waiting for Image Analysis to complete. Elapsed Time={}sec'
                         .format(int(time.time() - start_time_sec)))
            time.sleep(5)
    if time.time() - start_time_sec >= WAIT_TIMEOUT_SEC:
        raise TimeoutError("Timed out waiting for Image to Analyze (timeout={}sec)".format(WAIT_TIMEOUT_SEC))
    else:
        _logger.info('Image Analysis Complete, wait time: {}sec'.format(int(time.time() - start_time_sec)))