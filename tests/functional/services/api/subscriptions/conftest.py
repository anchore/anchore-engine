...
def add_alpine_subscription(request):
    ...
    def remove_subscription():
        resp = http_del(['subscriptions', subscription.get('subscription_id')], config=request.param)
        if resp.code != 200:
...
def add_subscription(api_conf: callable):
    ...
    if resp.code == 500 and resp.body.get('message') == 'subscription already exists in DB':
        # Already exists
        resp = http_get(['subscriptions'], config=api_conf)
        subscription_list = resp.body
        for subscription in subscription_list:
            if subscription.get('subscription_type') == 'tag_update' and subscription.get(
                    'subscription_key') == 'docker.io/alpine:latest':
