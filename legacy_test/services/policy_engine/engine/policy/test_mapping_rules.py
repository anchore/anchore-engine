import unittest
from anchore_engine.services.policy_engine.engine.policy.bundles import PolicyMappingRule
...
class TestPolicyMappingRules(unittest.TestCase):
    def test_tag_mapping(self):
        test_rules = [
            ...
            {
                # Tag match failure
                'rule': matcher_for_tag(registry='docker.io', repository='mysql', tag='latest'),
                'id': '0',
                'digest': 'sha256:123abc',
            ...
            },
            {
                ...
                'rule': matcher_for_tag(registry='docker.io', repository='library/nginx'),
                'id': '0',
                'digest': 'sha256:123abc',
                'tag': 'docker.io/nginx:latest',
                'match': False
            ...
            {
                # Exact match
                'rule': matcher_for_tag(registry='docker.io', repository='library/nginx', tag='latest'),
                'id': '0',
                'digest': 'sha256:123abc',
        ...
        for test in test_rules:
            ...
            self.assertEqual(test['match'], m, 'Failed on: {} with tag {}'.format(test['rule'], test['tag']))
    ...
    def test_digest_mapping(self):
        ...
        for test in test_rules:
            ...
            test_img.digest = test['digest']
            m = rule.matches(test_img, tag=test['tag'])
            self.assertEqual(test['match'], m, 'Failed on: {} with digest {}'.format(test['rule'], test['digest']))
    ...
    def test_id_mapping(self):
        ...
        for test in test_rules:
            ...
            test_img.digest = test['digest']
            m = rule.matches(test_img, tag=test['tag'])
            self.assertEqual(test['match'], m, 'Failed on: {} with id {}'.format(test['rule'], test['id']))
