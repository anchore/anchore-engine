...
from anchore_engine.subsys.object_store.config import ANALYSIS_ARCHIVE_MANAGER_ID, DEFAULT_OBJECT_STORE_MANAGER_ID, default_config as DEFAULT_OBJ_STORE_CONFIG, DRIVER_SECTION_KEY, DRIVER_NAME_KEY, extract_config as obj_store_extract_config
...
def extract_config(service_config, default=None):
    ...
    archive_obj_config = obj_store_extract_config(service_config=service_config, config_keys=[ANALYSIS_ARCHIVE_MANAGER_ID])
...
def initialize(config):
    ...
    if _manager_singleton is None:
        try:
            try:
                logger.debug('Initializing the analysis-archive object store driver: {}'.format(config.get(DRIVER_SECTION_KEY, {}).get(DRIVER_NAME_KEY)))
                archive_obj_config = extract_config(config)
                if archive_obj_config:
                    # There is an archive-specific configuration
                    initialize_direct(archive_obj_config, manager_id=ANALYSIS_ARCHIVE_MANAGER_ID)
...
class ArchiveManager(object):
    ...
    def __init__(self, object_manager_id):
        ...
        logger.debug('Archive manager initialized using object storage driver: {}'.format(self.obj_manager.primary_client.__config_name__))
    ...
    def get_document_meta(self, account, bucket, archiveId) -> dict:
        return self.obj_manager.get_document_meta(account, self._bucket(bucket), archiveId)
    ...
    def delete_document(self, account, bucket, archiveId) -> bool:
        return self.obj_manager.delete_document(account, self._bucket(bucket), archiveId)
