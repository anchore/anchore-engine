...
class PolicyEngineLoadFail(Event):
    __event_type__ = 'analysis_load_fail'
    __resource_type__ = _image_digest_resource_type

    def __init__(self, user_id, image_digest, error=None):
        super(PolicyEngineLoadFail, self).__init__(user_id=user_id, level='ERROR', message='Failed to load image analysis to policy engine', resource_id=image_digest, details=error)
