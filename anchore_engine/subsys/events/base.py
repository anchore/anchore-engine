...
CategoryDescriptor = namedtuple('CategoryDescriptor', field_names=['name', 'description'])
...
class EventMeta(type):
    ...
    def registered_categories(cls):
        return list(set([x.__category__ for x in cls.registry if hasattr(x, '__category__')]))

    def registered_subcategories(cls):
        return list(set([x.__subcategory__ for x in cls.registry if hasattr(x, '__subcategory__')]))
...
class EventBase(object, metaclass=EventMeta):
    ...
    def fq_event_type(cls):
        return '{}.{}.{}'.format(cls.__category__.name if cls.__category__ else '', cls.__subcategory__.name if cls.__subcategory__ else '', cls.__event_type__).lower()
