...
from anchore_engine.db import db_accounts, db_account_users, AccountTypes, UserAccessCredentialTypes
    ...
    def initialize_system_identities(self):
        ...
        try:
            ...
            if not self.mgr.get_account(localconfig.SYSTEM_ACCOUNT_NAME):
                self.mgr.create_account(localconfig.SYSTEM_ACCOUNT_NAME, AccountTypes.service, 'system@system', creator=localconfig.SYSTEM_IDENTITY_BOOTSTRAPPER)

            if not self.mgr.get_user(localconfig.SYSTEM_USERNAME):
                self.mgr.create_user(localconfig.SYSTEM_ACCOUNT_NAME, localconfig.SYSTEM_USERNAME, creator_name=localconfig.SYSTEM_IDENTITY_BOOTSTRAPPER)
                self.mgr.add_user_credential(creator_name=localconfig.SYSTEM_IDENTITY_BOOTSTRAPPER, username=localconfig.SYSTEM_USERNAME, credential_type=UserAccessCredentialTypes.password)
        ...
        try:
            if not self.mgr.get_account(localconfig.ADMIN_ACCOUNT_NAME):
                self.mgr.create_account(localconfig.ADMIN_ACCOUNT_NAME, AccountTypes.admin, 'admin@system',
                                        creator=localconfig.SYSTEM_IDENTITY_BOOTSTRAPPER)
            ...
            if not self.mgr.get_user(localconfig.ADMIN_USERNAME):
                self.mgr.create_user(localconfig.ADMIN_ACCOUNT_NAME, localconfig.ADMIN_USERNAME,
                                     creator_name=localconfig.SYSTEM_IDENTITY_BOOTSTRAPPER)

                self.mgr.add_user_credential(creator_name=localconfig.SYSTEM_IDENTITY_BOOTSTRAPPER,
...
class IdentityManager(object):
    ...
    # These are all unit-of-work functions. They manage their own db transaction

    def create_account(self, account_name, account_type, email, creator, is_active=True):
        """
