...
class AnchoreIOFeedClient(IFeedClient, Oauth2AuthenticatedClient):
    """
...
def get_client(feeds_url=None, token_url=None, client_url=None, user=tuple(), conn_timeout=None, read_timeout=None):
    ...
    if not conn_timeout:
        conn_timeout = conf['feeds'].get('connection_timeout_seconds')

    if not read_timeout:
        read_timeout = conf['feeds'].get('read_timeout_seconds')

    if not feeds_url:
        feeds_url = conf['feeds']['url']

    if not token_url:
        token_url = conf['feeds']['token_url']

    if not client_url:
        client_url = conf['feeds']['client_url']
    ...
    if not user:
        user = conf['feeds'].get('anonymous_user_username')
        password = conf['feeds'].get('anonymous_user_password')
