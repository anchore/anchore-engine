...
def query_vulnerabilities(id=None, page=1, limit=None, affected_package=None, affected_package_version=None, namespace=None):
    ...
    try:
        ...
        try:
            ...
            timer = time.time()
            result = client.query_vulnerabilities(vuln_id=request_inputs.get('params', {}).get('id'),
...
def query_images_by_vulnerability(vulnerability_id=None, severity=None, namespace=None, affected_package=None, page=1, limit=None, vendor_only=True):
    ...
    try:
        ...
        try:
            ...
            client = internal_client_for(PolicyEngineClient, ApiRequestContextProxy.namespace())
            timer = time.time()
            pe_result = client.query_images_by_vulnerability(user_id=ApiRequestContextProxy.namespace(), vulnerability_id=vulnerability_id, severity=severity, namespace=namespace, affected_package=affected_package, vendor_only=vendor_only)
            policy_engine_call_time = time.time() - timer
...
def query_images_by_package(name=None, version=None, package_type=None, page=1, limit=None):
    ...
    try:
        ...
        try:
            ...
            client = internal_client_for(PolicyEngineClient, ApiRequestContextProxy.namespace())
            timer = time.time()
            pe_result = client.query_images_by_package(user_id=ApiRequestContextProxy.namespace(), name=params.get('name'), version=params.get('version'), package_type=params.get('package_type'))
            policy_engine_call_time = time.time() - timer
