...
def create_analysis_archive_rule(rule):
    ...
    if not rule.get('system_global') and rule.get('max_images_per_account', None) is not None:
        raise BadRequest('Cannot set max_images_per_account on a rule that isn\'t system_global', {})
