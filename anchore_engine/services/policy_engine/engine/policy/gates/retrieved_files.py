...
from anchore_engine.services.policy_engine.engine.policy.params import SimpleStringParameter, EnumStringParameter
...
class RetrievedFileMixin(object):
    ...
    file_path = SimpleStringParameter(name='path', example_str='/etc/httpd.conf', description='The path of the file to verify has been retrieved during analysis', is_required=True, sort_order=1)
...
class FileNotStoredTrigger(BaseTrigger, RetrievedFileMixin):
    ...
    __description__ = 'Triggers if the specified file is not present/stored in the evaluated image.'
...
class FileContentRegexMatchTrigger(BaseTrigger, RetrievedFileMixin):
    ...
    regex = SimpleStringParameter(name='regex', example_str='.*SSlEnabled.*', description='The regex to evaluate against the content of the file', is_required=True)
    check = EnumStringParameter(name='check', example_str='match', enum_values=['match', 'no_match'], description='The type of check to perform with the regex', is_required=True)
    ...
    def evaluate(self, image_obj, context):
        ...
        try:
            ...
            logger.exception('Could not decode/process file content for {} in image {}/{} to do regex check'.format(path, image_obj.user_id, image_obj.id))
            raise Exception('Cannot execute regex check due to error processing file content')
        ...
        if match_found == (check_type == 'match'):
            self._fire(instance_id=self._construct_match_id(), msg="Content regex '{}' check '{}' found in retrieved file '{}'".format(re_value, check_type, path))
...
class RetrievedFileChecksGate(Gate):
    ...
    def prepare_context(self, image_obj, context):
        ...
        retrieved_file_contents = image_obj.analysis_artifacts.filter(AnalysisArtifact.analyzer_id == 'retrieve_files', AnalysisArtifact.analyzer_artifact == 'file_content.all', AnalysisArtifact.analyzer_type == 'base').all()
