...
class FullMatchTrigger(BaseTrigger):
    ...
    license_blacklist = CommaDelimitedStringListParameter(name='licenses', example_str='GPLv2+,GPL-3+,BSD-2-clause', description='Comma-delimited list of license names to blacklist exactly', is_required=True)
...
class SubstringMatchTrigger(BaseTrigger):
    ...
    licenseblacklist_submatches = CommaDelimitedStringListParameter(name='licenses', example_str='LGPL,BSD', description='Comma-delimited list of strings to do substring match for blacklist', is_required=True)
