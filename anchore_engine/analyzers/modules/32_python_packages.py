...
try:
    ...
    pythondbdir = anchore_engine.analyzers.utils.python_prepdb_from_squashtar(
        unpackdir, squashtar, py_library_file
    )
        ...
        if allfiles[f]["type"] == "dir":
            ...
            for distribution in distributions:
                ...
                if distribution.project_name in omit_pymods:
                    print(
                        "INFO: skipping module ({}) as it is a member of omit list".format(
                    ...
                    try:
                        ...
                        el["origin"] = "N/A"
                        try:
                            el["metadata"] = distribution.get_metadata(
                        ...
                        traceback.print_exc()
                        print(
                            "WARN: could not extract information about python module from distribution - exception: "
                            + str(err)
                        )
                        el = {}
            ...
            if pkg_type == "python":
                try:
                    ...
                    try:
                        ...
                        print(
                            "WARN: unable to add python package ({}) from hints - exception: {}".format(
                                pkg_key, err
                            )
                        )
                except Exception as err:
