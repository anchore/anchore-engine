...
if distrodict["flavor"] == "RHEL":
    try:
        (
            rpms,
            rpmdbdir,
        ) = anchore_engine.analyzers.utils.rpm_get_all_packages_detail_from_squashtar(
            unpackdir, os.path.join(unpackdir, "squashed.tar")
        )
    ...
    try:
        (
            ...
            other_packages,
            dpkgdbdir,
        ) = anchore_engine.analyzers.utils.dpkg_get_all_packages_detail_from_squashtar(
            unpackdir, os.path.join(unpackdir, "squashed.tar")
        )
    ...
    try:
        apkgs = anchore_engine.analyzers.utils.apkg_get_all_pkgfiles_from_squashtar(
            unpackdir, os.path.join(unpackdir, "squashed.tar")
        )
